  ...
  if(model.type =="IntcptReg" || model.type == "ffANOVA") 
  {
    ...
    for(i in 1:length(half_life_values))
    {
      ...
      { 
        ...
        #if (determinant_V==0) {print("Determinant of variance matrix ix 0, and is set to 1.0e-323"); determinant_V <-1.0e-323}
        gof[i, k] <- -N/2*log(2*pi)-0.5*log.det.V-0.5*(t(resid) %*% V.inverse%*%resid);
        if(gof[i, k]=="NaN") gof[i, k]<--10^100  #### Ask Thomas about this one ####
    ...
    if(model.type=="rReg")   
    {
      ...
      {  
        ...
        { 
          ...
          print(as.numeric(round(cbind(if(a!=0)log(2)/a else 0.00, vy, gof[i,k], t(beta1)), 4))) # Will increasing the number of digits avoid problems when plotting grid?
