...
for(p in 1:N)
{
  for(q in 1:N)
  {
    cm2[p,q]<-(((1-exp(-a*T.term[p]))/(a*T.term[p]))*((1-exp(-a*T.term[q]))/(a*T.term[q]))-(exp(-a*tia[p, q])*(1-exp(-a*T.term[q]))/ (a*T.term[q])+ exp(-a*tia[p, q])*(1-exp(-a*T.term[p]))/(a*T.term[p]))*(num.prob[p,q]));
  }
}

## New version
cm2_vectorized = calc.cm2(a, T.term, N, tia, ta)
