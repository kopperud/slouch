...
phy <- artiodactyla@phylo
n <- length(phy$tip.label)
nodes_internal <- (n+1):nrow(phy$edge)
nodes_tip <- 1:n
node_order <- order(as.numeric(artiodactyla@stats$node))
diet <- data.frame(species = phy$tip.label,
                   diet = artiodactyla@stats$diet[node_order][nodes_tip])
phy$node.label <- artiodactyla@stats$diet[node_order][nodes_internal]
...
model0 <- slouch.fit(phy = phy,
                     hl_values = seq(0, 12, length.out = 10),
...
model1 <- slouch.fit(phy = phy,
                     hl_values = seq(0, 150, length.out = 25),
...
model3 <- slouch.fit(phy = phy,
                     species = neocortex$species,
                     response = neocortex$neocortex_area_mm2_log_mean,
                     fixed.cov = neocortex$brain_mass_g_log_mean,
                     lower = c(0, 0.01),
                     hillclimb = TRUE)
plot(model3)
...
model4 <- 
  slouch.fit(phy = phy,
             species = neocortex$species,
             response = neocortex$neocortex_area_mm2_log_mean,
             fixed.cov = cbind(brain = neocortex$brain_mass_g_log_mean, 
...
model6 <- slouch.fit(phy = phy,
                     hl_values = seq(0, 5, length.out = 25),
...
model7 <- slouch.fit(phy = phy,
                     hl_values = seq(0, 4, length.out = 25),
                     vy_values = seq(0.001, 0.05, length.out = 25),
                     species = neocortex$species,
                     response = neocortex$neocortex_area_mm2_log_mean,
